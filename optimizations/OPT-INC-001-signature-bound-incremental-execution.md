...
- X: target-supported signature definitions, persistence scopes, invalidation granularities, output-validity/consumption policies, immutable-output handles, immutable-input snapshot/mutation-control policies, monotonic mutation epochs, and crash-consistent state-publication mechanisms
...
- f: measured repeated-work cost including stage runtime plus signature/snapshot/mutation-tracking/metadata/output-validation/output-pinning/publication I/O overhead
- d: minimize
...
- Variables: categorical / mixed policy choices for signatures, snapshots, validation, output pinning, granularity, mutation control, and publication
